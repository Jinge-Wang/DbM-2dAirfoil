...
system('rm ./Tmp/morphed_original.txt &> /dev/null');
fid = fopen('./Tmp/morphed_original.txt', 'w');
for i = 1:numel(M(:,1))
...
%     system('del /f XFOIL\morphed_repanel.txt > NUL');
    system('rm ./Tmp/morphed_repanel.txt &> /dev/null');
    fid = fopen('./Tmp/morphed_repanel.txt', 'w');
