...
fid = fopen('./Tmp/xfoilInput.input', 'w');
for i = 1:numel(A)
    if A{i+1} == -1
        fprintf(fid,'%s', A{i});
        break 
...
system('rm ./Tmp/xfoilSave.txt &> /dev/null');
system('rm ./Tmp/xfoilDump.txt &> /dev/null');

% system('xfoil.exe < XFOIL\xfoilInput.input')
% results=evalc("system('xfoil.exe < XFOIL\xfoilInput.input > NUL')");
results=evalc("system('./xfoil < ./Tmp/xfoilInput.input &> /dev/null')");

% OPEN XFOIL OUTPUT
fid = fopen('./Tmp/xfoilSave.txt');
if fid == -1
    output = 0;
    return
end
